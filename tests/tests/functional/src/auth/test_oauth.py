...
async def test_oauth_success(aiohttp_session) -> None:
...
@pytest.mark.asyncio(loop_scope='session')
async def test_yandex_oauth_success(aiohttp_session) -> None:
    authorize_url = urljoin(settings.auth_api_v1_url, 'oauth/yandex/authorize/')
    ...
    state = authorization_query_data['state'][0]
    callback_url = urljoin(settings.auth_api_v1_url, 'oauth/yandex/callback/')
    ...
    assert response_data['login'] is None
    assert response_data['email'] == 'test.user@yandex.ru'
    assert response_data['is_superuser'] == False
